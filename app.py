...
def deposit():
    if request.method == 'POST':
        ...
        if amount < 500:
            ...
            return render_template('deposit.html', user=g.user)
        import time as _t
        ref = f"DEP{g.user['id']}{int(_t.time())}"
...
def withdraw():
    if request.method == 'POST':
        ...
        if g.user['balance'] < amount:
            ...
            return render_template('withdraw.html', user=g.user)
        import time as _t
        ref = f"WIT{g.user['id']}{int(_t.time())}"
        execute("""INSERT INTO transactions
                   (user_id,type,amount,status,reference,note) VALUES (?,?,?,?,?,?)""",
...
def admin_balance(uid):
    ...
    log_admin('balance_edit', f"User {user['username']}: {action} {fmt(amount)} TZS")
...
def admin_toggle_user(uid):
    ...
    log_admin('toggle_user', f"{'Activated' if new else 'Suspended'} {user['username']}")
...
def admin_set_match_score(mid):
    ...
    if match:
        log_admin('set_score',
                  f"Match #{mid} ({match['home_code']} v {match['away_code']}): preset {home}-{away}")
...
def admin_force_start_route(mid):
    ...
    if match:
        log_admin('force_start',
                  f"Force-started #{mid} ({match['home_code']} v {match['away_code']})")
...
def admin_force_finish_route(mid):
    ...
    if match:
        log_admin('force_finish',
                  f"Force-finished #{mid} ({match['home_code']} v {match['away_code']})")
...
def admin_predictions():
    ...
    for m in upcoming:
        ...
        cs   = odds.get('cs', {})
        best = min(((k,v) for k,v in cs.items() if k!='other'),
...
# ══════════════════════════════════════════════════════════════════════════
def seed():
    """Create admin + demo accounts and first matchdays if they don't exist."""
    import logging
    log = logging.getLogger(__name__)

    try:
        existing_admin = query("SELECT id FROM users WHERE username='admin'", one=True)
        ...
        existing_demo = query("SELECT id FROM users WHERE username='demo'", one=True)
        ...
        existing_md = query("SELECT id FROM matchdays LIMIT 1", one=True)
        if not existing_md:
            create_next_matchday(app)
            create_next_matchday(app)
            log.info("Initial matchdays created.")
